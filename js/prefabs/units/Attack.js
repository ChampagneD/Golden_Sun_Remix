...
RPG.Attack.prototype.show_message = function (target, damage) {
    ...
    // show attack message
    action_message_position = new Phaser.Point(this.game_state.game.width / 2, this.game_state.game.height * 0.1);
    action_message_text = this.owner.name + " attacks " + target.name + " with " + damage + " damage";
    attack_message = new RPG.ActionMessage(this.game_state, this.name + "_action_message", action_message_position, {group: "hud", texture:     "rectangle_image", scale: {x: 1.5, y: 0.2}, duration: 1, message: action_message_text});
};