...
RPG.MagicSpellMenuItem = function (game_state, name, position, properties) {
    ...
    this.SpellName = properties.spell.name; 
...
RPG.MagicSpellMenuItem.prototype.select = function () {
    ...
    if (this.game_state.current_unit.stats.mana >= this.MANA_COST) {
        ...
        // save current attack
        this.game_state.current_attack = new RPG.MagicAttack(this.game_state, this.game_state.current_unit.name + "_attack", {x: 0, y: 0}, {group: "attacks", mana_cost: this.MANA_COST, damage: this.damage, owner_name: this.game_state.current_unit.name, name: this.SpellName});
    }
};
